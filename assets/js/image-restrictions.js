jQuery( document ).ready( function( $ ) {
	...
	const settings = WP_Plugin_Starter_ImageRestrictions ?? {
			...
			if ( Boolean( settings.enableDimensions ) ) {
				...
				img.onload = function() {
					...
					if ( this.width !== settings.requiredWidth || this.height !== settings.requiredHeight ) {
						reject(
							`Image must be exactly ${ settings.requiredWidth }x${ settings.requiredHeight } pixels. Your image is ${ this.width }x${ this.height } pixels.`
						);
						return;
					}
					resolve( true );
