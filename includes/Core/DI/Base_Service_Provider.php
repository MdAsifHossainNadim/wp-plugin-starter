...
use WPPluginStarter\Dependencies\League\Container\ServiceProvider\AbstractServiceProvider;
 ...
 * @since 3.13.0
 */
abstract class Base_Service_Provider extends AbstractServiceProvider {
	/**
	...
	public function provides( string $alias ): bool {
		...
		// Direct match in services or tags
		if ( in_array( $alias, $this->services, true ) || in_array( $alias, $this->tags, true ) ) {
			return true;
		}

		// Look at implemented interfaces
	 ...
	 */
	protected function add_with_implements_tags( string $id, $concrete = '', bool $shared = false ): DefinitionInterface {
		$definition = $this->getContainer()->add( $id, $concrete )->setShared( $shared );

		if ( is_string( $concrete ) && class_exists( $concrete ) ) {
			foreach ( class_implements( $concrete ) as $interface ) {
	 ...
	 * @return void
	 */
	protected function add_tags( DefinitionInterface $definition, array $tags ): void {
		foreach ( $tags as $tag ) {
